define([
    'jquery',
    'EffectConnect_Marketplaces/js/lib/moment',
    ...
    var numberList = function(numbers) {
        ...
        return numbers.join(', ') + ' ' + $.mage.__('and') + ' ' + moment()._locale.ordinal(last_val);
    };
    ...
    var dateList = function(numbers, type) {
        ...
        return output_text + ' ' + $.mage.__('and') + ' ' + numberToDateName(last_val, type);
    };
    ...
    var scheduleToSentence = function(schedule) {
        ...
        if (schedule['h'] && schedule['m'] && schedule['h'].length <= 2 && schedule['m'].length <= 2) {
            ...
            if (hm.length < 2) {
                ...
                var last_val = hm.pop();
                output_text = hm.join(', ') + ' ' + $.mage.__('and') + ' ' + last_val;
            }
            if (!schedule['d'] && !schedule['D']) {
                output_text += ' ' + $.mage.__('every day');
            }
            ...
            if(schedule['h']) { // runs only at specific hours
                if (schedule['m']) { // and only at specific minutes
                    output_text += numberList(schedule['m']) + ' ' + $.mage.__('minute past the') + ' ' + numberList(schedule['h']) + ' ' + $.mage.__('hour');
                } else { // specific hours, but every minute
                    output_text += $.mage.__('minute of') + ' ' + numberList(schedule['h']) + ' ' + $.mage.__('hour');
                }
            } else if(schedule['m']) { // every hour, but specific minutes
                if (schedule['m'].length == 1 && schedule['m'][0] == 0) {
                    output_text += $.mage.__('hour, on the hour');
                } else {
                    output_text += numberList(schedule['m']) + ' ' + $.mage.__('minute past every hour');
                }
            } else { // cronspec has "*" for both hour and minute
                output_text += $.mage.__('minute');
            }
        }

        if (schedule['D'] && schedule['D'].length !== 31) { // runs only on specific day(s) of month
            output_text += ' ' + $.mage.__('on the') + ' ' + numberList(schedule['D']);
            if (!schedule['M']) {
                output_text += ' ' + $.mage.__('of every month');
            }
        }
        ...
        if (schedule['d']) { // runs only on specific day(s) of week
            if (schedule['D']) {
                ...
                output_text += ' ' + $.mage.__('and every') + ' ';
            } else {
                output_text += ' ' + $.mage.__('on') + ' ';
            }
        ...
        if (schedule['M']) {
            if( schedule['M'].length === 12 ) {
                output_text += ' ' + $.mage.__('day of every month')
            } else {
                // runs only in specific months; put this output last
                output_text += ' ' + $.mage.__('in') + ' ' + dateList(schedule['M'], 'mon');
            }
        }
